    ...
    <div class="headtitle">
        <div class="btn-group">
            ...
            <ul class="dropdown-menu">
                <li><a href="javascript:void(0);" class="get-html" data-route="{{action('Lab\ServiceController@index')}}">{{trans('labels.all')}}</a></li>
                <li class="divider"></li>
            ...
            @endif
                ...
                <td class="center">
                    <a href="javascript:void(0);" class="change-flag {{$homepage = $el->homepage ? 'colorGreen' : 'colorRed'}}" data-route="{{action('Lab\ServiceController@changeFlag', array($el->id, 'homepage'))}}">                        
                        <i class="fa fa-circle" aria-hidden="true" data-placement="top" data-original-title="{{trans('labels.homepage')}} ?"></i>
                    </a>
                </td>
                <td>{{$el->id}}</td>
                <td>{{$el->title}}</td>
                ...
                <td class="right">
                    <div class="btn-group btn-group-xs">
                        ...
                        <button href="javascript:void(0);" class="btn btn-danger delete-json" data-route="{{action('Lab\ServiceController@destroy', array($el->id))}}" data-token="{{ csrf_token() }}" data-callback="$(btn).closest('tr').remove()" data-confirm="{{trans('labels.confirm-delete')}}"><i class="fa fa-fw fa-trash" aria-hidden="true"></i></button>
                    </div>                                
                </td>
            </tr>
            @endforeach
