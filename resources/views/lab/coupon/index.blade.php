    ...
    <div class="headtitle">
        <div class="btn-group">
            ...
            <ul class="dropdown-menu">
                <li><a href="javascript:void(0);" class="get-html" data-route="{{action('Lab\CouponController@index')}}">{{trans('lab.all')}}</a></li>
                <li class="divider"></li>
            ...
            @endif
                ...
                <td class="center">
                    <a href="javascript:void(0);" class="change-flag {{$multipleusers = $el->multipleusers ? 'colorGreen' : 'colorRed'}}" data-route="{{action('Lab\CouponController@changeFlag', array($el->id, 'multipleusers'))}}">                        
                        <i class="fa fa-circle tip" aria-hidden="true" data-placement="top" data-original-title="{{trans('lab.multipleusers')}} ?"></i>
                    </a>
                </td>
                <td>{{$el->id}}</td>
                <td>{{$el->title}}</td>
                ...
                <td class="right">
                    <div class="btn-group btn-group-xs">
                        ...
                        <button href="javascript:void(0);" class="btn btn-danger delete-json" data-route="{{action('Lab\CouponController@destroy', array($el->id))}}" data-token="{{ csrf_token() }}" data-callback="$(btn).closest('tr').remove()" data-confirm="{{trans('lab.confirm-delete')}}"><i class="fa fa-fw fa-trash" aria-hidden="true"></i></button>
                    </div>                                
                </td>
            </tr>
            @endforeach
