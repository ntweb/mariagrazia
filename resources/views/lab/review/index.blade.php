    ...
    <div class="headtitle">
        <div class="btn-group">
            ...
            <ul class="dropdown-menu">
                <li><a href="javascript:void(0);" class="get-html" data-route="{{action('Lab\ReviewController@index')}}">{{trans('lab.all')}}</a></li>
                <li class="divider"></li>
    ...
    <table class="table table-striped responsive">
        <thead>
            <tr>
                ...
                <th>{{trans('lab.name')}}</th>
                <th>{{trans('lab.type')}}</th>
                <th>{{trans('lab.vote')}}</th>
                <th class="right">ID EL:</th>
                <th>{{trans('lab.element')}}</th>
            ...
            <tr>
                ...
                <td class="right">
                    <div class="btn-group btn-group-xs">
                        ...
                        <button href="javascript:void(0);" class="btn btn-danger delete-json" data-route="{{action('Lab\ReviewController@destroy', array($el->id))}}" data-token="{{ csrf_token() }}" data-callback="$(btn).closest('tr').remove()" data-confirm="{{trans('lab.confirm-delete')}}"><i class="fa fa-fw fa-trash" aria-hidden="true"></i></button>
                    </div>                                
                </td>
            </tr>
            @endforeach
