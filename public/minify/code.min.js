...
function getHtml(route,container,callback,confirmmsg){!function($){return"undefined"==typeof container&&(container=maincontainer),!("undefined"!=typeof confirmmsg&&!confirm(confirmmsg))&&(spinner(container),void $.get(route,function(data){$(container).html(data),"undefined"!=typeof callback&&eval(callback+";"),initUI()},"html"))}(jQuery)}function getHtmlNoSpinner(t,e){!function(a){"undefined"==typeof e&&(e=maincontainer),a.get(t,function(t){a(e).html(t),initUI()},"html")}(jQuery)}function getJson(route,btn,callback){!function($){var txt=btn.html();btn.html("wait..."),$.get(route,function(data){return btn.html(txt),data.error?void popUpError(data.error):(callback&&(data.result&&(callback=callback.replace("param",data.result)),eval(callback+";")),void(data.success&&popUpSuccess(data.success)))},"json")}(jQuery)}function deleteJson(route,token,btn,callback){!function($){var txt=btn.html();btn.html("wait..."),$.ajax({url:route,type:"DELETE",dataType:"json",headers:{"x-csrf-token":token},success:function(data){return btn.html(txt),data.error?void popUpError(data.error):(callback&&(data.result&&(callback=callback.replace("param",data.result)),eval(callback+";")),void(data.success&&popUpSuccess(data.success)))}})}(jQuery)}function spinner(t){!function(e){data='<div style="margin-left: 10px; margin-top: 5px;"><i class="fa fa-circle-o-notch fa-spin spinner" aria-hidden="true"></i> wait...</div>',e(t).html(data)}(jQuery)}function waitButton(t,e){!function(a){if(e){t.hide(0);var n=a('<div style="margin-left: 10px; margin-top: 5px;"><i class="fa fa-circle-o-notch fa-spin spinner" aria-hidden="true"></i> wait...</div>');n.insertBefore(t)}else a(".spinner").remove(),t.show(0)}(jQuery)}function simpleError(t,e){var a=t.find(".alert-danger").first();return a.length>0?(a.html(e),a.show(0),!0):void popUpError(e)}function fieldsError(t,e){!function(a){a.each(e,function(e,n){var r=t.find("*[name='"+e+"']").first();r.closest(".form-group").addClass("has-error"),a("<span>").addClass("label").addClass("label-danger").addClass("pull-left").addClass("margin-top2px").text(n).insertAfter(r)})}(jQuery)}function popUpError(t){alert(t)}function simpleSuccess(t,e){var a=t.find(".alert-success").first();return a.length>0?(a.html(e),a.show(0),!0):void popUpSuccess(e)}function popUpSuccess(t){}function hideErrorsSuccess(){!function(t){t(".alert-danger").hide(0),t(".alert-sucess").hide(0),t(".label-danger").remove(),t("*").removeClass("has-error"),t("*").removeClass("warning")}(jQuery)}function initUI(){!function(t){}(jQuery)}var maincontainer="#rpc-container";!function($){$(document).on("ready",function(){if($(document).on("submit","form.ns",function(e){e.preventDefault();var frm=$(this),btn=frm.find("*[type='submit']").first(),route=frm.data("route");if("undefined"==typeof route)return alert("error: no route detected."),!1;var check=frm.find("*[type='checkbox'][name='confirm']").first();if(check.length&&!check.is(":checked"))return popUpError("Please confirm."),!1;var dataserialized=frm.serialize(),callback=frm.data("callback");"undefined"==typeof callback&&(callback=null);var method=frm.data("method");if("undefined"==typeof method&&(method="POST"),"POST"==method)var dataserialized=new FormData(frm[0]),processData=!1,contentType=!1;else var dataserialized=frm.serialize(),processData=!1,contentType="application/x-www-form-urlencoded,multipart/form-data,text/plain";var btnOldValue=btn.html();btn.attr("disabled","disabled"),btn.html("Wait.."),waitButton(btn,!1);var dataType="json";$.ajax({method:method,url:route,data:dataserialized,cache:!1,processData:processData,contentType:contentType,dataType:dataType,timeout:6e4,error:function(t,e,a){popUpError("timeout"==e?"Timeout error":t.status+" "+a)},beforeSend:function(){hideErrorsSuccess()},success:function(data){if(data.error&&simpleError(frm,data.error),data.errorfields&&fieldsError(frm,data.errorfields),data.error||data.errorfields)return!1;callback&&(data.result&&(callback=callback.replace("param",data.result)),eval(callback+";")),data.redirect&&window.location.replace(data.redirect),data.success&&simpleSuccess(frm,data.success);var _ismodified=frm.find("input[name='_ismodified']").first();_ismodified.length&&_ismodified.val(0)},complete:function(){btn.html(btnOldValue),btn.removeAttr("disabled"),waitButton(btn,!1),frm.hasClass("ns-reset")&&frm[0].reset()}})}),$(document).on("submit",".form-get-html",function(t){t.preventDefault();var e=$(this).data("route"),a=$(this).data("container"),n=$(this).data("callback"),r=$(this).data("confirm"),o=$(this).serialize();e=e.indexOf("?")<0?e+"?"+o:e+"&"+o,getHtml(e,a,n,r)}),$(document).on("click",".get-html",function(t){t.preventDefault();var e=$(this),a=e.data("route"),n=e.data("container"),r=$(this).data("callback"),o=$(this).data("confirm");getHtml(a,n,r,o)}),$(document).on("click",".get-json",function(t){t.preventDefault();var e=$(this),a=e.data("route"),n=e.data("callback");"undefined"==typeof n&&(n=null),getJson(a,e,n)}),$(document).on("click",".delete-json",function(t){t.preventDefault();var e=$(this),a=e.data("route"),n=e.data("callback"),r=e.data("token");"undefined"==typeof n&&(n=null),"undefined"!=typeof e.data("confirm")?confirm(e.data("confirm"))&&deleteJson(a,r,e,n):deleteJson(a,r,e,n)}),$(document).on("change","._ismodified input, ._ismodified select, ._ismodified textarea",function(){var t=$(this).closest("form");if($(this).addClass("warning"),t.length){var e=t.find("input[name='_ismodified']").first();e.length&&e.val(1)}}),$(document).on("submit","form.ns-disable-submit",function(t){$("button",$(this)).attr("disabled","disabled")}),$("*[data-widget='map']").length&&$("*[data-widget='infowindow']").length){var widget=$("*[data-widget='infowindow']").first(),txt_infowindow=widget.html(),txt_lat=parseFloat(widget.data("lat")),txt_lng=parseFloat(widget.data("lng")),zoom=16,myLatlng=new google.maps.LatLng(txt_lat,txt_lng),mapOptions={zoom:zoom,center:myLatlng},infowindow=new google.maps.InfoWindow({content:txt_infowindow}),map=new google.maps.Map($("*[data-widget='map']").first()[0],mapOptions),marker=new google.maps.Marker({position:myLatlng,map:map});map.setZoom(zoom),infowindow.open(map,marker)}}),$(document).on("click",".callback",function(e){var callback=$(this).data("callback");eval(callback+";")}),$(document).on("click",".close-upload-meta",function(){$("#upload-meta").html(""),$("#upload-plugin").show(0)}),$("input[name='city']").geocomplete({types:["(cities)"]}).bind("geocode:result",function(t,e){$(this).prevAll("input[name='place_id']").first().val(e.place_id);var a=$(this).prevAll("input[name='political_short_name']").first(),n=$(this).prevAll("input[name='country_short_name']").first(),r=new google.maps.Geocoder;r.geocode({placeId:e.place_id},function(t,e){if(e===google.maps.GeocoderStatus.OK){if(t[0]){var r=t[0].address_components;$(r).each(function(t,e){e.types.indexOf("country")>=0?n.val(e.short_name):e.types.indexOf("political")>=0&&a.val(e.short_name)})}}else window.alert("Geocoder failed due to: "+e)})})}(jQuery);
!function(t){t(document).on("ready",function(){function a(){var a=t('*[data-cart-widget="cart_refresh"]').first(),r=a.data("route");t.get(r,function(t){a.html(t)},"html")}t(document).on("click",'*[data-cart-widget="mod_qty"]',function(){var a=parseInt(t(this).data("v")),r=t(".target-"+t(this).data("target")).first(),i=parseInt(r.val())+a;i>0&&r.val(i);var e=t(".cart-form-"+t(this).data("target")).first(),o=e.find('input[name="qty"]').first();o.val(r.val())}),t(document).on("click",'*[data-cart-widget="add"]',function(){var r=t(t(this).data("target")).first(),i=r.serialize(),e=r.data("route"),o=t(this),n=o.html();o.html("wait..."),o.attr("disabled","disabled"),t.post(e,i,function(t){o.html(n),o.removeAttr("disabled"),t.error&&alertify.error(t.error),t.success&&(alertify.success(t.success),a())},"json")}),t(document).on("click",'*[data-cart-widget="delete"]',function(){var r=t(this).data("route"),i=t(this).data("v");t.get(r,function(r){r.error?alertify.error(r.error):(t(i).remove(),r.location?window.location.replace(r.location):a())},"json")}),t(document).on("click",'*[data-cart-widget="mod_color"]',function(){var a=t(this).data("v"),r=t(t(this).data("target")).first();r.find('input[name="color"]').first().val(a)}),t(document).on("click",'*[data-cart-widget="mod_size"]',function(){var a=t(this).data("v"),r=t(t(this).data("target")).first();r.find('input[name="size"]').first().val(a)})})}(jQuery);